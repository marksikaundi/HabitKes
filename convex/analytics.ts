...
export const getHabitAnalytics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const completionRate =
      totalDays > 0 ? (filteredCompletions.length / totalDays) * 100 : 0;
    ...
    return {
      ...
      completions: filteredCompletions.sort((a, b) =>
...
export const getUserAnalytics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const allCompletions = await ctx.db.query("habitCompletions").collect();
    ...
    const mostConsistentHabit = findMostConsistentHabit(
      userHabits,
      userCompletions
    );
    ...
    return {
      ...
      averageStreakLength:
        userStreaks.length > 0
          ? Math.round(
              userStreaks.reduce((sum, s) => sum + s.currentStreak, 0) /
                userStreaks.length
            )
          : 0,
...
export const getWeeklyAnalytics = query({
  ...
  handler: async (ctx, args) => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const dayCompletions = userWeekCompletions.filter(
        (c) => c.date === dateStr
      );
      const completionRate =
        userHabits.length > 0
    ...
    return {
      ...
      dailyStats,
      perfectDays: dailyStats.filter((day) => day.completionRate === 100)
        .length,
    };
  },
});
...
function findMostConsistentHabit(habits: any[], completions: any[]): any {
  ...
  const habitStats = habits.map((habit) => {
    ...
    const consistency =
      daysSinceStart > 0 ? (habitCompletions.length / daysSinceStart) * 100 : 0;
