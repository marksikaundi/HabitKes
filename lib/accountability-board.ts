...
function readConfig(): AppwriteConfig | null {
  ...
  const habitsCollectionId =
    process.env.EXPO_PUBLIC_APPWRITE_HABITS_COLLECTION_ID ?? "";
  const friendsCollectionId =
    process.env.EXPO_PUBLIC_APPWRITE_FRIENDS_COLLECTION_ID ?? "";
  const activityCollectionId =
    process.env.EXPO_PUBLIC_APPWRITE_ACTIVITY_COLLECTION_ID ?? "";
...
function mapHabit(document: any): Habit {
  ...
  const progress = Number(
    document.progress ??
      Math.min(100, Math.round((streak / Math.max(bestStreak, 1)) * 100)),
  );
...
function mapActivity(document: any): Activity {
  return {
    ...
    tone:
      document.tone === "warning" || document.tone === "neutral"
        ? document.tone
        : "positive",
  };
}
...
export function useAccountabilityBoard(): AccountabilityBoard {
  ...
  const refreshBoard = useCallback(async () => {
    ...
    try {
      const [habitResponse, friendResponse, activityResponse] =
        await Promise.all([
      ...
      setHabits(
        habitResponse.documents.length > 0
          ? habitResponse.documents.map(mapHabit)
          : seedHabits,
      );
      setFriends(
        friendResponse.documents.length > 0
          ? friendResponse.documents.map(mapFriend)
          : seedFriends,
      );
      setActivity(
        activityResponse.documents.length > 0
          ? activityResponse.documents.map(mapActivity)
          : seedActivity,
      );
    ...
    const watch = async () => {
      ...
      for (const channel of channels) {
        try {
          ...
          setConnectionState("error");
          setConnectionLabel(
            "Realtime subscription failed, staying on local data",
          );
        }
      }
        ...
        current.map((habit) => {
          ...
          updatedHabit = {
            ...
            progress: Math.min(
              ...
              Math.max(
                10,
                completedToday ? habit.progress + 5 : habit.progress - 8,
              ),
            ),
          };
  ...
  const addFriend = useCallback(
    async (name: string, focus: string) => {
      ...
      try {
        ...
        setConnectionState("error");
        setConnectionLabel(
          "Appwrite friend invite failed, keeping the local board",
        );
      }
    },
    [pushActivity, services],
